...
export const columns = (
...
): ColumnDef<Team>[] => [
  {
    accessorKey: "sport",
    header: "Team Name",
    cell: ({row}) => <span>{row.original.sport +" "+ row.original.grade +" "+ row.original.gender}</span>,
  },
  {
    accessorKey: "teachers",
    header: "Emails",
    cell: ({row}) => <div className="w-[900px] truncate whitespace-normal break-words">{row.original.teachers.join(", ")}</div>,
  },
//   {
//     accessorKey: "season",
//     header: "Season",
...
//   },
  {
    ...
    header: ({ table }) => {
        ...
        .getRowModel()
        .rows.every((row) => selectedTeams.includes(row.original.id.toString()));
        ...
        return (
            <div className="flex items-center justify-end gap-3 text-right pr-4">
                ...
                <input
                    ...
                    if (allSelected) {
                        // Uncheck all
                        setSelectedTeams((prev) => prev.filter((id) => !ids.includes(id)));
                    } else {
                        // Check all
